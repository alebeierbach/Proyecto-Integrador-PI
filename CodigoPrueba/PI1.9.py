...
def leer_humedad_suelo_pct(canal_adc):
    try:
        ...
        _ = canal_adc.value  # dummy

        muestras = []
        for _i in range(5):
            muestras.append(canal_adc.value)
            time.sleep(0.1)

        raw16 = sum(muestras) / len(muestras)
        GPIO.output(PIN_SENSOR_POWER, GPIO.LOW)

        lectura_8bit = int(raw16 / 256)  # 0..255
        humedad_pct = int((lectura_8bit - RAW_SECO) * 100 / (RAW_MOJADO - RAW_SECO))
        humedad_pct = max(0, min(100, humedad_pct))

        return humedad_pct, lectura_8bit

    except Exception as e:
        GPIO.output(PIN_SENSOR_POWER, GPIO.LOW)
        print(f"Error leyendo humedad suelo: {e}")
        return 0, 0
    ...
    try:
        with open(NOMBRE_ARCHIVO, mode='a', newline='') as f:
            ...
            if not existe:
                ...
                writer.writerow(["Fecha", "Hora", "Lux Natural", "Lux Total", "Hum Suelo %", "Hum Suelo RAW", "Temp Amb C", "Hum Amb %", "Luz Estado", "Riego Estado"])
...
def subir_datos_iot(lux_total, lux_natural, hum_suelo, hum_suelo_raw, temp_amb, hum_amb, esta_regando):
    try:
        payload = {
...
def main():
    ...
    try:
        while True:
            ...
            # -------------------------------------------------------
            # 1. LOGICA DE LUZ INTELIGENTE (Ambiente y Artificial)
            # -------------------------------------------------------
            
            # A. Apagamos foco y medimos Luz Ambiente
            GPIO.output(PIN_LUZ, GPIO.HIGH) 
            time.sleep(2) # Esperamos que el sensor se estabilice sin foco
            try: lux_natural = sensor_bh1750.lux
            except: lux_natural = 0
            
            # B. Decidimos Estado (Soporta cruce de medianoche)
            hora_actual = ahora.hour
            estado_luz = "OFF"

            # Evaluamos si estamos en la franja de horas activas
            if HORA_INICIO_LUZ < HORA_FIN_LUZ:
                es_de_dia = (hora_actual >= HORA_INICIO_LUZ) and (hora_actual < HORA_FIN_LUZ)
            else:
                # Horario invertido que cruza la medianoche (ej. 18 a 6)
                es_de_dia = (hora_actual >= HORA_INICIO_LUZ) or (hora_actual < HORA_FIN_LUZ)

            if es_de_dia:
                if lux_natural < UMBRAL_LUX:
                    GPIO.output(PIN_LUZ, GPIO.LOW) # PRENDER FOCO (Rele Activo en BAJO)
                    estado_luz = "ON"
                else:
                    estado_luz = "OFF"
            else:
                # Si no estamos en el horario activo, nos aseguramos que este apagado
                GPIO.output(PIN_LUZ, GPIO.HIGH) # APAGAR FOCO
                estado_luz = "OFF"

            # C. Medimos Luz Total (Lo que realmente recibe la planta)
            time.sleep(2) # Esperamos que el foco ilumine bien (si se prendio)
            ...
            # 2. OTRAS LECTURAS
            # -------------------------------------------------------
            hum_suelo, hum_suelo_raw = leer_humedad_suelo_pct(adc_humedad)
            if dht_device: temp_amb, hum_amb = leer_ambiente(dht_device)
            else: temp_amb, hum_amb = 0, 0

            print(f"Ambiente: {temp_amb:.1f}C/{hum_amb:.0f}% | Suelo: {hum_suelo}% (raw={hum_suelo_raw})")
            ...
            guardar_excel([
                ...
                hum_suelo, hum_suelo_raw,
                temp_amb, hum_amb, estado_luz, estado_riego_str
            ])
            
            # A la nube subimos el TOTAL (que es lo que le importa a la planta)
            subir_datos_iot(int(lux_total), int(lux_natural), hum_suelo, hum_suelo_raw, temp_amb, hum_amb, riego_activo_bool)
            # 5. ESPERA
